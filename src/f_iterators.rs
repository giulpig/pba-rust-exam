...
pub fn bounded_absolute_values(vals: impl Iterator<Item = i32>) -> impl Iterator<Item = u32> {
	...
	//Vec::new().into_iter()
	vals.map(|x| x.unsigned_abs()).filter(|&x| x <= 100)
}
...
impl Iterator for Fibonacci {
	...
	fn next(&mut self) -> Option<u32> {
		// Sane imperative implementation
		/*
		match (self.prev_prev, self.prev) {
			(Some(pp), Some(p)) => {
				self.prev_prev = Some(p);
				self.prev = Some(p + pp);
			}
			(None, Some(0)) => {
		...
		*/
		...
		// Not-so-sane functional implementation (just for fun)
		Some(*self.prev.insert(
			self.prev_prev.unwrap_or(0)
				+ self.prev.unwrap_or(0)
				+ (self.prev.is_some_and(|x| *self.prev_prev.insert(x) == 0)) as u32,
		))
	}
}
