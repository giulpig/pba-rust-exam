...
impl<Name, Id> TypedEmployeeBuilder<Name, Id> {
	pub fn experience(self, experience: u32) -> Self {
		//todo!("finish the implementation. Note that you might need to move some of these
		// functions to a new `impl` blocks with different trait bounds, or change the return type
		// to use `Named` etc.");
		Self {
			name: self.name,
			uid: self.uid,
