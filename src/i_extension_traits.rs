...
pub fn process_stuff<T>(_input: impl Iterator<Item = T>) -> Vec<Outcome> {
...
pub fn something_went_wrong_count(outcomes: Vec<Outcome>) -> usize {
	...
	outcomes
		.iter()
		.filter(|&x| matches!(*x, Outcome::SomethingWentWrong))
		.count()
}
pub fn i_dont_know_count(outcomes: Vec<Outcome>) -> usize {
	...
	outcomes
		.iter()
		.filter(|&x| matches!(*x, Outcome::IDontKnow))
		.count()
}
...
impl OutcomeCount for Vec<Outcome> {
	fn ok_count(&self) -> usize {
		//todo!();
		self.iter().filter(|&x| matches!(*x, Outcome::Ok)).count()
	}
	fn i_dont_know_count(&self) -> usize {
		//todo!();
		self.iter()
