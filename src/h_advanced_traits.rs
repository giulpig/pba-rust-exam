...
pub trait ProvideEnergy<F: Fuel> {
	...
	fn provide_energy_with_efficiency(&self, f: FuelContainer<F>, e: u8) -> <F as Fuel>::Output {
		//todo!();
		let e = std::cmp::min(e, 100) as u32;
		(F::energy_density().into() * f.amount * e / 100).into()
	}
...
impl ProvideEnergy<Uranium> for NuclearReactor {
	fn provide_energy(&self, f: FuelContainer<Uranium>) -> <Uranium as Fuel>::Output {
		//todo!("complete the implementation; note that you might need to change the trait bounds
		// and generics of the `impl` line");
...
impl<const DECAY: u32> InternalCombustion<DECAY> {
	pub fn new(efficiency: u8) -> Self {
		//todo!()
		Self {
			efficiency: RefCell::new(std::cmp::min(100, efficiency)),
			decay_counter: RefCell::new(0),
		}
	}
}
...
impl<const DECAY: u32> ProvideEnergy<Diesel> for InternalCombustion<DECAY> {
	fn provide_energy(&self, f: FuelContainer<Diesel>) -> <Diesel as Fuel>::Output {
		...
		// Calculate the provided energy
		let energy = self.provide_energy_with_efficiency(f, *self.efficiency.borrow());

		// Update decay counter
		if self
			.decay_counter
			.replace_with(|&mut old| (old + 1) % DECAY)
			== DECAY - 1
		{
...
impl<const C: u8, F1: Fuel, F2: Fuel> Fuel for CustomMixed<C, F1, F2> {
	...
	fn energy_density() -> Self::Output {
		//todo!("complete the implementation; note that you might need to change the trait bounds
		// and generics of the `impl` line");
		(F1::energy_density().into() * C as u32 + F2::energy_density().into() * (100 - C as u32))
			/ 100
	}
}
...
pub fn omni_80_energy(amount: u32) -> BTU {
	//todo!();
	(OmniGenerator::<80> {})
		.provide_energy(FuelContainer::new(amount) as FuelContainer<Mixed<Diesel, LithiumBattery>>)
}
...
impl<F> ProvideEnergy<F> for GreenEngine<F>
where
	F: Fuel + IsRenewable,
{
	fn provide_energy(&self, f: FuelContainer<F>) -> <F as Fuel>::Output {
...
impl<F> ProvideEnergy<F> for BritishEngine<F>
where
	F: Fuel<Output = BTU>,
{
	fn provide_energy(&self, f: FuelContainer<F>) -> <F as Fuel>::Output {
